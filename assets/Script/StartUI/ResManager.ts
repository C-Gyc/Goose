...
import { Asset, SpriteAtlas, SpriteFrame, Prefab, JsonAsset, AudioClip, resources, Constructor, AssetManager, assetManager  } from "cc";
interface AssetMap {
    ...
    addRes(asset: Asset): void {
        if (asset instanceof SpriteAtlas) {
            ...
            for (const frame of asset.getSpriteFrames()) {
                this._assets.frames.set(frame.name, frame);
            }
            return;
        }
        // 判断资源类型是不是我们需要的
